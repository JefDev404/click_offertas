...
// SALVAR / ATUALIZAR SLIDER FIT
router.post("/", async (req, res) => {
  const { pagina, tipo, nome, preco, imagem, link, ordem } = req.body;

  if (!pagina || !tipo || !preco || !imagem || !link || ordem === undefined) {
  ...
  try {
    ...
    console.error("Erro ao salvar sliderFit:", err);
...
router.get("/", async (req, res) => {
  try {
    ...
    console.error("Erro ao buscar sliderFit:", err);
